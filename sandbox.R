...
a$body <- gsub('^close','',a$body)
a$body <- trimws(a$body,"b")
...
pc[,firstDay := (comment_timestamp - timestamp < 1440)]
pc <- pc[firstDay == T]

# Aggregate comments and count per post_id
c_condensed <- c[,.(comment_count = length(comment_id)),by=post_id]
